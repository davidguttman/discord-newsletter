...
const { TOPIC_EXTRACTION_PROMPT, FOCUSED_STORY_PROMPT, CATCHALL_STORY_PROMPT, NEWSPAPER_ASSEMBLY_PROMPT } = require('./newspaper-prompts')
...
async function generateMultiPassSummary (messages, model, maxTokens, options = {}) {
  ...
  const catchallResponse = await openai.createChatCompletion({
    model,
    messages: [
      {
        role: 'system',
  ...
  const catchallContent = catchallResponse.data.choices[0].message.content
  log(`📝 Generated community highlights: ${catchallContent.substring(0, 100)}...`, 'catchall-generated')

  // Add catchall usage to tracking
  totalUsage.prompt_tokens += catchallResponse.data.usage.prompt_tokens
  totalUsage.completion_tokens += catchallResponse.data.usage.completion_tokens
  totalUsage.total_tokens += catchallResponse.data.usage.total_tokens

  // Pass 4: Assemble final newspaper
  log('📰 Pass 4: Assembling final newspaper...', 'assembly-start')
  const allContent = mainStoriesContent + '\n\n' + catchallContent

  const assemblyPrompt = NEWSPAPER_ASSEMBLY_PROMPT
    .replace('{stories}', allContent)
  ...
  let finalSummary = finalResponse.data.choices[0].message.content
