...
const messageTagsRouter = require("./api/message-tags");
...
const health = healthpoint(function (cb) {
  mongoose
    .checkHealth()
    .then(() => cb())
    .catch(cb);
...
if (process.env.NODE_ENV === "test") {
  ...
  app.use("/tags", mockAuth, tagsRouter);
  app.use("/message-tags", mockAuth, messageTagsRouter);
} else {
  app.use("/messages", messagesRouter);
  app.use("/summarize", summarizeRouter);
  app.use("/email-summary", emailSummaryRouter);
  app.use("/tags", tagsRouter);
  app.use("/message-tags", messageTagsRouter);
}
